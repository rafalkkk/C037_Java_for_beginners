...
public class Trip {
    ...
    public Trip() {
        System.out.println("Trip: default constructor");
    }

    public Trip(String tripDestination, int tripDuration,
                int tripRating, int tripEstimatedCost) {
        System.out.println("Trip: constructor with parameters");
        destination = tripDestination;
        duration = Math.max(tripDuration, 1);
        rating = Math.max(Math.min(tripRating, TripUtils.getMaxRatio()), TripUtils.getMinRatio());
        estimatedCost = tripEstimatedCost;
        totalEstimatedCost += tripEstimatedCost;
    }

    public Trip(Trip trip) {
        System.out.println("Trip: copy constructor");
    ...
    public void setRating(int rating) {
        this.rating = Math.max(Math.min(rating, TripUtils.getMaxRatio()), TripUtils.getMinRatio());
    }
